...
public class Funcionario1 {
    ...
    public Funcionario1() throws ExcecaoAlfabetoEspaco, ExcecaoNumeroPositivo {
        setNome(NOME_POR_OMISSAO);
        setVencimento(VENC_POR_OMISSAO);
    }

    public Funcionario1(String nome, float vencimento) throws ExcecaoAlfabetoEspaco, ExcecaoNumeroPositivo {
        setNome(nome);
    ...
    public void setNome(String nome) throws ExcecaoAlfabetoEspaco{
        ...
        for(int i=0;i<nome.length();i++){
            c = nome.charAt(i);
            if (!Character.isLetter(c) && Character.isSpaceChar(c)== false)
    ...
    public void setVencimento(float vencimento) throws ExcecaoNumeroPositivo{
        this.vencimento = vencimento;
        if (vencimento < 0){
            throw new ExcecaoNumeroPositivo("O vencimento: " + vencimento + " não pode ser negativo.");}
    }
